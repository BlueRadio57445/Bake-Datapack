execute if score @s vehicles.sandwich.fly matches 1 if score @s vehicles.moving matches 0 store result entity @s Rotation[0] float 1 run scoreboard players get $p.x_rot vehicles.calc
execute if score @s vehicles.sandwich.fly matches 1 if score @s vehicles.moving matches 0 run data modify entity @s Rotation[1] set value 0.0f
