...
execute as @a if predicate weapons:gloves/hold_main_gloves if score @s weapons.gloves.drop matches 1.. run function weapons:gloves/dropped_off_gloves

# 如果主手有主要拳套，副手卻沒有次要拳套 ---> 儲存原本的副手物品，給予次要拳套
execute as @a if predicate weapons:gloves/hold_main_gloves unless predicate weapons:gloves/hold_off_gloves run function weapons:gloves/return_off_gloves
