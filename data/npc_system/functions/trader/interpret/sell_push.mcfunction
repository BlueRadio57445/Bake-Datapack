...
data modify storage general:gcm str_list append from storage npc_system:trader Interpret.Sell[0].buy.Name
...
data modify storage general:gcm str_list append from storage npc_system:trader Interpret.Sell[0].buyB.Name
function general:gcm/push/server/string_list

data modify storage general:gcm str_list set value ["loot replace block 0 16 0 container.2 1 loot "]
data modify storage general:gcm str_list append from storage npc_system:trader Interpret.Sell[0].sell.Name
function general:gcm/push/server/string_list

data modify storage general:gcm str set value "execute as @e[limit=1,type=minecraft:marker,scores={npc.trader_interpret=-1}] run function npc_system:trader/interpret/sell_pull"
