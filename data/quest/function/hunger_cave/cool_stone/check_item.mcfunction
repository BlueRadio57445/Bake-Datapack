# executed by marker
execute store success score $item_check npc.state run clear @a[limit=1,tag=npc.user] prismarine_crystals[custom_data~{cool_stone:{}}] 0
...
execute if score $item_check npc.state matches 1 run clear @a prismarine_crystals[custom_data~{cool_stone:{}}]
