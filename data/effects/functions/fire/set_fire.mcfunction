...
execute if score $exceedHealth effects.state matches 1.. run function effects:fire/add_max_health
execute at @s run summon minecraft:area_effect_cloud ~ ~ ~ {Radius:0f,Duration:6,Age:4,ReapplicationDelay:1,effects:[{id:"minecraft:instant_health",amplifier:2b,duration:1,show_icon:0b,show_particles:0b}]}
