execute store result score $durability.unbreaking general.extra_lore run data get storage general:utils Item.tag.Enchantments[{id:"minecraft:unbreaking"}].lvl
...
execute store result score $durability.damage general.extra_lore run data get block 0 16 0 Items[{Slot:1b}].tag.effects[0].duration
