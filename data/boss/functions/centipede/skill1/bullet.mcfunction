...
execute as @e[tag=centipede.bullet] at @s run particle end_rod ~ ~ ~ 0 0 0 0.1 0 force
execute as @e[tag=centipede.bullet] at @s run damage @a[limit=1,sort=nearest,distance=0..1] 5 magic by @s
execute if score $bullet centipede.ability matches 101.. run kill @e[tag=centipede.bullet]
execute if score $bullet centipede.ability matches ..100 run schedule function boss:centipede/skill1/bullet 1t
