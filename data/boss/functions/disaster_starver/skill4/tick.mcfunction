...
execute as @s[scores={boss.time1=14}] run function boss:disaster_starver/skill4/summon_villager

# 只要王想打玩家，就讓村民拉仇恨
execute as @s[scores={boss.time1=15..}] at @s on target if entity @s[type=player] as @e[type=husk,tag=disaster_starver.clawing] run function boss:disaster_starver/skill4/villager_attack
